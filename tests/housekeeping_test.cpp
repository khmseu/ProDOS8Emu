...
int main() {
  ...
  // Test 1: CREATE a standard file with file_type and aux_type
  {
    ...
    prodos8emu::write_u16_le(mem.banks(), paramBlock + 10,
                             0x0000);  // create_time (0 = use current)
  ...
  // Test 11: ON_LINE with unit_num != 0 returns single volume or ERR_NO_DEVICE
  {
    ...
    if (err != prodos8emu::ERR_NO_DEVICE) {
      std::cerr << "FAIL: Expected ERR_NO_DEVICE for non-existent unit, got 0x" << std::hex
                << (int)err << "\n";
      failures++;
    } else {
  ...
  // Test 12: Pathname > 64 bytes returns ERR_INVALID_PATH_SYNTAX
  {
    ...
    if (err != prodos8emu::ERR_INVALID_PATH_SYNTAX) {
      std::cerr << "FAIL: Expected ERR_INVALID_PATH_SYNTAX for >64 byte pathname, got 0x"
                << std::hex << (int)err << "\n";
      failures++;
    } else {
  ...
  // Test 13: Relative path with empty prefix returns ERR_INVALID_PATH_SYNTAX
  {
    ...
    if (err != prodos8emu::ERR_INVALID_PATH_SYNTAX) {
      std::cerr
          << "FAIL: Expected ERR_INVALID_PATH_SYNTAX for relative path with empty prefix, got 0x"
          << std::hex << (int)err << "\n";
      failures++;
    } else {
  ...
  // Test 14: Corrupted metadata xattr falls back to defaults
  {
    ...
    if (err != prodos8emu::ERR_NO_ERROR) {
      ...
      err = ctx.getFileInfoCall(mem.banks(), getBlock);
      if (err != prodos8emu::ERR_NO_ERROR) {
        std::cerr << "FAIL: GET_FILE_INFO on corrupted metadata returned error 0x" << std::hex
                  << (int)err << "\n";
        failures++;
      } else {
        uint8_t file_type    = prodos8emu::read_u8(mem.banks(), getBlock + 4);
  ...
  // Test 16: ON_LINE caps at 14 volumes + terminator
  {
    ...
    if (err != prodos8emu::ERR_NO_ERROR) {
      ...
      if (volumeCount != 14) {
        ...
        if (terminator != 0) {
          std::cerr << "FAIL: Expected terminator at record 14, got byte0=0x" << std::hex
                    << (int)terminator << "\n";
          failures++;
        } else {
  ...
  // Test 18: Permission denied error
  {
    ...
    fs::permissions(readOnlyVol, fs::perms::owner_read | fs::perms::owner_exec,
    ...
    if (err != prodos8emu::ERR_ACCESS_ERROR) {
      std::cerr << "FAIL: Expected ERR_ACCESS_ERROR for read-only directory, got 0x" << std::hex
                << (int)err << "\n";
      failures++;
    } else {
