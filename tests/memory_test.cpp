...
int main() {
  ...
  // Test 5b: uint16_t cross-bank boundary at 0x0FFF
  {
    ...
    // Should span banks: 0x0FFF (bank 0) stores low byte 0x78, 0x1000 (bank 1) stores high byte
    // 0x56
  ...
  // Test 9: Read ProDOS counted string with maxLen enforcement
  {
    ...
    if (str.length() != 10) {
      std::cerr << "FAIL: read_counted_string maxLen not enforced, got " << str.length()
                << " chars\n";
      failures++;
    } else {
