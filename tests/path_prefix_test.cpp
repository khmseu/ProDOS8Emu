...
int main() {
    ...
    // Test 5: Pathname too long returns ERR_INVALID_PATH_SYNTAX
    {
        ...
        uint8_t err = ctx.setPrefixCall(mem.constBanks(), 0x0300);
        if (err != prodos8emu::ERR_INVALID_PATH_SYNTAX) {
            std::cerr << "FAIL: Expected ERR_INVALID_PATH_SYNTAX for long path, got " << std::hex << (int)err << "\n";
            failures++;
        } else {
    ...
    // Test 6: Component starts with digit returns ERR_INVALID_PATH_SYNTAX
    {
        ...
        uint8_t err = ctx.setPrefixCall(mem.constBanks(), 0x0300);
        if (err != prodos8emu::ERR_INVALID_PATH_SYNTAX) {
            std::cerr << "FAIL: Expected ERR_INVALID_PATH_SYNTAX for digit start, got " << std::hex << (int)err << "\n";
            failures++;
        } else {
    ...
    // Test 7: xattr roundtrip (or returns ERR_IO_ERROR if unsupported)
    {
        ...
        if (err == prodos8emu::ERR_IO_ERROR) {
            std::cout << "PASS: xattr operations return ERR_IO_ERROR (filesystem doesn't support xattrs)\n";
    ...
    // Test 11: Partial pathname with empty prefix returns ERR_INVALID_PATH_SYNTAX
    {
        ...
        uint8_t err = ctx.setPrefixCall(mem.constBanks(), 0x0300);
        if (err != prodos8emu::ERR_INVALID_PATH_SYNTAX) {
            std::cerr << "FAIL: Expected ERR_INVALID_PATH_SYNTAX for partial path with empty prefix, got " << std::hex << (int)err << "\n";
            failures++;
        } else {
