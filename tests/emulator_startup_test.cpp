...
int main() {
  ...
  std::string tempDirName = "prodos8emu_emulator_startup_test_" + 
                            std::to_string(pid) + "_" + 
  ...
  try {
    ...
    if (warmAddr != 0x2000) {
      std::cerr << "FAIL: Warm start vector at $03F2 should be $2000, got $" 
                << std::hex << warmAddr << std::dec << "\n";
      failures++;
    }
    
    if (warmPowerUpByte != 0xA5) {
      std::cerr << "FAIL: Warm start power-up byte at $03F4 should be $A5, got $" 
                << std::hex << (int)warmPowerUpByte << std::dec << "\n";
      failures++;
    }
    ...
    if (resetVec != 0x2000) {
      std::cerr << "FAIL: Reset vector at $FFFC should be $2000, got $" 
                << std::hex << resetVec << std::dec << "\n";
      failures++;
    }
    ...
    if (cpu.regs().pc != 0x2000) {
      std::cerr << "FAIL: PC after reset should be $2000, got $" 
                << std::hex << cpu.regs().pc << std::dec << "\n";
      failures++;
    }
    ...
    if (instructionCount != 2) {
      std::cerr << "FAIL: Expected 2 instructions (JMP + STP), executed " 
                << instructionCount << "\n";
      failures++;
    } else {
    ...
    if (cpu.regs().pc != 0x2004) {
      std::cerr << "FAIL: PC after STP should be $2004, got $" 
                << std::hex << cpu.regs().pc << std::dec << "\n";
      failures++;
    } else {
