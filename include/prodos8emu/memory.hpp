 ...
 */
inline std::string read_counted_string(const MemoryBanks& banks, uint16_t addr, std::size_t maxLen = 64) {
 ...
 * @return The string data (count byte is not included)
 */
inline std::string read_counted_string(const ConstMemoryBanks& banks, uint16_t addr, std::size_t maxLen = 64) {
    uint8_t count = read_u8(banks, addr);
    std::size_t actual_len = std::min(static_cast<std::size_t>(count), maxLen);
