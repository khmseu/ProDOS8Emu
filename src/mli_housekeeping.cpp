...
// Date format: bits 0-4: day (1-31), bits 5-8: month (1-12), bits 9-15: year (0-127, offset from 1900)
...
uint8_t storeMetadata(const std::filesystem::path& hostPath, const ProDOSMetadata& meta) {
    ...
    oss << std::setw(2) << (int)meta.access << ":"
        << std::setw(2) << (int)meta.file_type << ":"
        << std::setw(4) << meta.aux_type << ":"
        << std::setw(2) << (int)meta.storage_type << ":"
        << std::setw(4) << meta.create_date << ":"
        << std::setw(4) << meta.create_time << ":"
        << std::setw(4) << meta.mod_date << ":"
        << std::setw(4) << meta.mod_time;
 ...
 */
std::string readPathname(const ConstMemoryBanks& banks, uint16_t paramBlockAddr, uint16_t offset) {
...
uint8_t MLIContext::onLineCall(MemoryBanks& banks, uint16_t paramBlockAddr) {
    ...
    if (unit_num != 0) {
        ...
        for (size_t j = 0; j < volName.length(); j++) {
            write_u8(banks, static_cast<uint16_t>(data_buffer + 1 + j), static_cast<uint8_t>(volName[j]));
        }
    ...
    int recordCount = 0;
    size_t maxVolumes = std::min(volumes.size(), static_cast<size_t>(14));  // Limit to 14 volumes (slots 1-7, drives 0-1) + terminator
    ...
    for (size_t i = 0; i < maxVolumes; i++) {
        ...
        // Write volume name
        for (size_t j = 0; j < volName.length(); j++) {
            write_u8(banks, static_cast<uint16_t>(bufferOffset + 1 + j), static_cast<uint8_t>(volName[j]));
        }
