...
CADIUS_SUFFIX_RE = re.compile(r"^(?P<stem>.*)#(?P<ft>[0-9A-Fa-f]{2})(?P<aux>[0-9A-Fa-f]{4})$")
...
def cmd_cadius_to_xattr(args: argparse.Namespace) -> int:
  ...
  for p in iter_paths([Path(x) for x in args.paths], recursive=args.recursive):
    ...
    try:
      ...
      if args.dry_run:
        print(f"XATTR  {p} {XATTR_FILE_TYPE}={ft_str} {XATTR_AUX_TYPE}={aux_str}")
      else:
...
def cmd_xattr_to_cadius(args: argparse.Namespace) -> int:
  ...
  for p in iter_paths([Path(x) for x in args.paths], recursive=args.recursive):
    ...
    suffix = f"#{ft_b:02X}{aux_w:04X}" if args.uppercase else f"#{ft_b:02x}{aux_w:04x}"
...
def build_parser() -> argparse.ArgumentParser:
  ...
  p1.add_argument("--keep-name", action="store_true", help="Do not rename files (keep #TTAAAA suffix)")
  p1.add_argument("--dry-run", action="store_true", help="Print actions without changing anything")
  p1.add_argument(
      "--follow-symlinks",
      action="store_true",
  ...
  p2.add_argument("--dry-run", action="store_true", help="Print actions without changing anything")
  p2.add_argument(
      "--overwrite",
      action="store_true",
